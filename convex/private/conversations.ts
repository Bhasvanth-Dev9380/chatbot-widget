...
export const updateStatus = mutation({
  args: {
    ...
    // ✅ pass orgId from the client (Better Auth → Convex user)
  ...
  handler: async (ctx, args) => {
    ...
    // ✅ ensure this conversation belongs to the provided organization
...
export const getOne = query({
  args: {
    conversationId: v.id("conversations"),
    organizationId: v.string(), // 👈 pass from client (Convex user orgId)
  },
  handler: async (ctx, args) => {
    const conversation = await ctx.db.get(args.conversationId);
    ...
    // Ensure the conversation belongs to this organization
    if (conversation.organizationId !== args.organizationId) {
      throw new ConvexError({
        code: "UNAUTHORIZED",
        message: "Invalid Organization ID",
...
export const getMany = query({
  args: {
    ...
    // ✅ pass orgId from the client (Better Auth)
    organizationId: v.string(),
  },
  handler: async (ctx, args) => {
